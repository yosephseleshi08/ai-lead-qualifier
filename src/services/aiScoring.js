...
try {
  ...
  if (process.env.REDIS_URL || process.env.REDIS_HOST) {
    ...
    redis.on('error', (err) => {
      logger.error('Redis connection error — using in-memory fallback', { error: err.message });
      useRedis = false;
    });
    ...
    setTimeout(() => {
      if (!useRedis) {
        logger.info('Redis connection timeout — using in-memory fallback');
        useRedis = false;
      }
    }, 5000);
  } else {
  ...
  logger.error('Failed to initialize Redis — using in-memory fallback', { error: err.message });
  useRedis = false;
}
...
const getInMemoryRateLimit = (tenantId) => {
  ...
  if (validTimestamps.length >= RATE_LIMIT_MAX) {
    return { allowed: false, current: validTimestamps.length, limit: RATE_LIMIT_MAX, resetAt: windowStart + RATE_LIMIT_WINDOW };
  }
  
  validTimestamps.push(now);
  memoryStore.set(key, validTimestamps);
  return { allowed: true, current: validTimestamps.length, limit: RATE_LIMIT_MAX };
};
...
const FALLBACK_SCORES = {
  ...
  talkingPoints: ['Review lead manually'],
  actionItems: ['Schedule follow-up call'],
  objections: [],
  competitorMentions: [],
  buyingSignals: [],
...
const checkRateLimit = async (tenantId) => {
  ...
  if (useRedis && redis) {
    try {
      ...
      logger.error('Redis rate limit check failed — using in-memory fallback', { error: err.message, tenantId });
      // Fall through to in-memory
    }
  }
  
  // In-memory fallback
  return getInMemoryRateLimit(tenantId);
};
...
const analyzeLead = async (lead, messages = [], tenantId = 'default') => {
  ...
  if (!rateLimit.allowed) {
    logger.warn(`AI scoring rate limit exceeded for tenant ${tenantId}`, { 
      current: rateLimit.current, 
      limit: rateLimit.limit 
    });
    return { 
      ...FALLBACK_SCORES, 
      aiSummary: `Rate limit exceeded (${rateLimit.current}/${rateLimit.limit} per hour). Using fallback scoring.`, 
      riskFactors: ['AI rate limit exceeded'] 
    };
  }
  ...
  if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === 'sk-your-key-here') {
    ...
    return { 
      ...FALLBACK_SCORES, 
      aiSummary: 'OpenAI API key not configured. Please set OPENAI_API_KEY in environment variables.',
      riskFactors: ['OpenAI API key missing']
    };
  }
  ...
  return breaker.execute(async () => {
    try {
      ...
      logger.error('OpenAI API error', { error: error.message, leadId: lead.id });
...
const quickScore = async (lead) => {
  ...
  return {
    ...
    sentiment: 0,
    urgencyScore: Math.floor(score * 0.3),
    intentScore: Math.floor(score * 0.4),
    budgetScore: Math.floor(score * 0.15),
    painScore: Math.floor(score * 0.15),
    authorityScore: 0,
    timingScore: 0,
    aiSummary: 'Quick heuristic score (AI detailed analysis pending)',
    talkingPoints: [],
    actionItems: ['Run full AI analysis'],
    objections: [],
    competitorMentions: [],
    buyingSignals: [],
    riskFactors: [],
...
const analyzeSentiment = async (text) => {
  // Check if OpenAI API key is configured
  if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === 'sk-your-key-here') {
    logger.warn('OpenAI API key not configured — using default sentiment');
    return { sentiment: 0, confidence: 0, emotions: [] };
  }
