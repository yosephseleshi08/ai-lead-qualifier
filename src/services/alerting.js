...
const detectAnomaly = async (tenantId, currentMetrics) => {
  ...
  try {
    ...
    logger.error('Failed to read anomaly state from Redis', { error: err.message });
  ...
  try {
    ...
    logger.error('Failed to persist anomaly state to Redis', { error: err.message });
  }
  ...
  if (currentMetrics.newLeads < state.baseline.volume * 0.5 && currentMetrics.newLeads > 0) {
    ...
    if (!state.lastAlert || now - state.lastAlert > 86400000) {
      state.lastAlert = now;
      try { await redis.set(key, JSON.stringify(state), 'EX', 86400 * 7); } catch (err) {}
      return {
